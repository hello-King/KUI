	...
	void CHorizontalLayoutUI::SetPos(RECT rc)
	{
		...
		for( int it2 = 0; it2 < m_items.GetSize(); it2++ ) {
			...
			if( sz.cx == 0 ) {
				...
				cxFixedRemaining -= sz.cx + rcPadding.left + rcPadding.right;//修复padding属性计算出错，导致子控件自动计算款高度错误，自身宽高计算错误
			}
			...
			RECT rcCtrl = { iPosX + rcPadding.left, rc.top + rcPadding.top, iPosX + sz.cx + rcPadding.left /*+ rcPadding.right*/, rc.top + rcPadding.top + sz.cy};////修复padding属性计算出错，导致子控件自动计算款高度错误，自身宽高计算错误
