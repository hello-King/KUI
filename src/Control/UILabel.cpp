...
namespace DuiLib
{
    ...
    {
        if ( m_bAutoWidth == bAutoWidth ) return;
        
        m_bAutoWidth = bAutoWidth;
        int nWidth = GetTextOutWidth();
        SetFixedWidth(nWidth + 4);
    }
	...
	void CLabelUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
        ...
        else if( _tcscmp(pstrName, _T("autowidth")) == 0){
            SetAutoWidth(_tcscmp(pstrValue, _T("true")) == 0);
        }
    ...
    int CLabelUI::GetTextOutWidth()
    {
        ...
        CRenderEngine::DrawText(hDC, m_pManager, rc, m_sText, m_dwTextColor, \
        m_iFont, DT_CALCRECT |DT_SINGLELINE | m_uTextStyle);
        return rc.right;
    }
}