...
bool CListUI::Remove(CControlUI* pControl)
{
    ...
    if( iIndex == m_iCurSel && m_iCurSel >= 0 ) {
        int iSel = m_iCurSel;
        m_iCurSel = -1;
        //SelectItem(FindSelectable(iSel, false));//heqing 自动选中下一个，导致删除所有的bug（itemchanged会自动选中复选框）
    }
...
{
    m_pOwner = static_cast<IListOwnerUI*>(pOwner->GetInterface(_T("IListOwner")));
    IListUI* pControl = static_cast<IListUI*>(pOwner->GetInterface(_T("IList")));
    m_pHeader = pControl->GetHeader();
}
