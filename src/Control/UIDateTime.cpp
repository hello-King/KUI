...
#define DT_KEEP   3
	...
	LRESULT CDateTimeWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		// 		}
        //else if (uMsg == WM_NOTIFY) //没法兼容xp与win7，故使用原生控件 addby heqing
        //{
            //NMHDR* pHeader=(NMHDR*)lParam;
            //HWND hWndDTP1=pHeader->hwndFrom;
            //if (pHeader->code==MCN_SELCHANGE)//这个事件在年份、月份、日期修改都会触发
            //{
            //    LPNMDAYSTATE lpChange = (LPNMDAYSTATE)lParam;
            //    DWORD day = lpChange->stStart.wDay;
            //    ::SendMessage(m_hWnd, DTM_SETSYSTEMTIME, GDT_VALID,(LPARAM)&(lpChange->stStart));
            //    memcpy((void*)&m_pOwner->m_sysTime, (void*)&lpChange->stStart, sizeof(SYSTEMTIME));
            //    m_pOwner->m_nDTUpdateFlag = DT_UPDATE;
            //    m_pOwner->UpdateText();
            //    ::PostMessage(hWndDTP1, WM_CLOSE, NULL, NULL);
            //    PostMessage(WM_CLOSE);
            //}
        //}
		else bHandled = FALSE;
    ...
    void CDateTimeUI::UpdateDateTime()
    {
        if (m_pWindow)
        {
            ...
            ::PostMessage(m_pWindow->GetHWND(),WM_CLOSE, NULL, NULL);
        }
        else
        {
            m_nDTUpdateFlag = DT_UPDATE;
